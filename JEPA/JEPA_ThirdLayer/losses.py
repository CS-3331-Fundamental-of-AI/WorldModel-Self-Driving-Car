...
# ---------------------------------------------------------
def inverse_affordance_losses(out, target_s_tg, target_s_tg_2=None):
    losses = {}
    s_tg_hat = out["s_tg_hat"]
    s_a = out["s_a_detached"]
    ...
    losses["total"] = (
        1.0 * losses["cos_s_tg_a"]
        + 0.5 * losses["l1_s_tg_a"]
        + 0.5 * losses.get("cos_s_tg_true", 0)
        + 0.5 * losses.get("l1_s_tg_true", 0)
        + 0.1 * losses["vic_s_y"]
        + 0.1 * losses["vic_z_ca"]
    )
    return losses
