...
class TwoWayEnv(AbstractEnv):
    ...
    """
    From: https://github.com/wuxiyang1996/Heterogeneous_Highway_Env/blob/master/envs/two_way_env.py
    ...
    def _is_terminated(self) -> bool:
        """Compatibility with newer highway_env that calls _is_terminated()."""
        return self._is_terminal()

    def _is_truncated(self) -> bool:
        """No time-limit truncation here; rely on wrapper for max steps."""
        return False
        ...
        for i in range(3):
            self.road.vehicles.append(
                vehicles_type(road,
                              ...
                              .position(70+40*i + 10*self.np_random.standard_normal(), 0),
                              heading=road.network.get_lane(("a", "b", 1)).heading_at(70+40*i),
                              speed=24 + 2*self.np_random.standard_normal(),
                              enable_lane_change=False)
            )
        for i in range(2):
            v = vehicles_type(road,
                              position=road.network.get_lane(("b", "a", 0))
                              .position(200+100*i + 10*self.np_random.standard_normal(), 0),
                              heading=road.network.get_lane(("b", "a", 0)).heading_at(200+100*i),
                              speed=20 + 5*self.np_random.standard_normal(),
                              enable_lane_change=False)
...
register(
    id="two-way-v0",
    # Use this local implementation rather than the highway_env package namespace.
    entry_point="envs.two_way_env:TwoWayEnv",
