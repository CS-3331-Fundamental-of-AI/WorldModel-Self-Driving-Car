...
# ✅ Robust: load .env from an absolute path (Colab-safe)
...
def main():
    ...
    for epoch_idx in range(int(EPOCH)):
        ...
        for step_idx, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch_idx+1}/{EPOCH}")):
            ...
            if (step_idx + 1) % ACCUM_STEPS == 0:
                ...
                # ✅ clip only predictor (encoders frozen)
                torch.nn.utils.clip_grad_norm_(
                    primitive_layer.predictor.parameters(), 1.0
                )
